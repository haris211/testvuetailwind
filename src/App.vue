<template>
  <div id="app" class="flex container h-screen w-full">
    ...
    <div class="lg:w-1/5 border-r border-lighter px-2 lg:px-6 py-2 flex flex-col justify-between">
      <div>
        <button class="h-12 w-12 hover:bg-lightblue text-3xl rounded-full text-sky-400">
          <i class="fab fa-twitter"></i>
        </button>
        <div>
          <button v-for="tab in tabs" @click="id = tab.id" :class="`focus:outline-none hover:text-sky-400 flex items-center py-2 px-4 hover:bg-lightblue rounded-full mr-auto mb-3 ${ id === tab.id ? 'text-blue' : ''}`">
            <i :class="`${ tab.icon } text-2xl mr-4 text-left`"></i>
            <p class="text-lg font-semibold text-left hidden lg:block"> {{ tab.title }} </p>
          </button>
        </div>
        <button class="text-white bg-sky-400 rounded-full font-semibold focus:outline-none w-12 h-12 lg:h-auto lg:w-full p-3 hover:bg-darkblue">
      ...
      <div class="lg:w-full relative">
        <button @click="dropdown = true" class="flex items-center w-full hover:bg-lightblue rounded-full p-2 focus:outline-none">
          ...
          <div class="hidden lg:block ml-4">
            <p class="text-sm font-bold leading-tight"> Shine Sietz Lion </p>
            <p class="text-sm leading-tight"> @SaeSiyEth </p>
          </div>
    ...
    <div class="w-full md:w-1/2 h-full overflow-y-scroll">
      ...
      <div class="px-5 py-3 border-b-8 border-lighter flex">
        ...
        <form v-on:submit.prevent = "addNewTweet" class="w-full px-4 relative">
          ...
          <button type="submit" class="h-10 px-4 text-white font-semibold bg-blue-400 hover:bg-darkblue focus:outline-none rounded-full absolute bottom-0 right-0">
            Tweet
          </button>
        </form>
      </div>
        ...
        <div v-for="tweet in tweets" class="w-full p-4 border-b hover:bg-lighter flex">
          ...
          <div class="w-full">
            <div class="flex items-center w-full">
              <p class="font-semibold"> Shine Sietz Lion </p>
              <p class="text-sm text-dark ml-2"> @SaeSiyEth </p>
...
export default {
  ...
  data() {
    return {
      ...
      dropdown: false,
      trending: [
        {top: 'Trending in TX', title: 'Gigi Hadid', bottom: 'Trending with: Create A New Song'},
        {top: 'Music', title: 'Finnaly Comeback', bottom: '135K Tweets'},
        {top: 'Pop', title: 'Sarah Ivy', bottom: '40k tweets'},
        {top: 'Trending in US', title: 'Jeans Day', bottom: '40k tweets'},
        {top: 'Trending', title: 'Mei Beyonce', bottom: '25.4k tweets'},
      ],
      friends: [
        {src: 'elon.jpg', name: 'Elon Brine', handle: '@lamboBoy'},
        {src: 'monk.jpg', name: 'Adrian Monk', handle:'@you:)'},
        {src: 'kevin.jpg', name: 'Bark Hart', handle: '@bigRock'}
      ],
      following: [
        {src: 'elon.jpg', name: 'Elon Brine', handle: '@lamboBoy', time: '20 min', tweet: 'Should I just quarantine on jupiter??', comments: '1,000', retweets: '550', like: '1,000,003'},
        {src: 'kevin.jpg', name: 'Kevin Hart', handle: '@bigRock', time: '55 min', tweet: 'Should me and the rock do another sub-par movie together????', comments: '2,030', retweets: '50', like: '20,003'},
        {src: 'elon.jpg', name: 'Elon Brine', handle: '@lamboBoy', time: '1.4 hr', tweet: 'Haha just made a flame thrower. Shld I sell them?', comments: '100,000', retweets: '1,000,002', like: '5,000,003'},
        {src: 'elon.jpg', name: 'Elon Brine', handle: '@lamboBoy', time: '1.4 hr', tweet: 'Just did something crazyyyyyyy', comments: '100,500', retweets: '1,000,032', like: '5,000,103'}
      ],
      tweets: [
        {content: 'It is so nice outsidem, and so hot!'}
      ],
      tweet: {content: ''}
    }
  },
